...
void loop() {
  ...
  if (scd4x.update()) {
    ...
    current_co2 = scd4x.getCO2();
    current_temp = scd4x.getTemperature();
    current_humidity = scd4x.getHumidity();

    // データ範囲チェック（異常値検知）
    if (current_co2 > 10000 || current_co2 < 300) {
      Serial.printf("WARNING: Abnormal CO2 value: %d ppm\n", current_co2);
      sensorErrorCount++;
    } else if (current_temp > 60.0 || current_temp < -20.0) {
